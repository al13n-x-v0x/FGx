...
const CANVAS_AVAILABLE = (() => {
  try {
    ...
    return typeof createCanvas === 'function';
  } catch {
    return false;
  }
})();
 ...
 *   - Member count badge
 *
 * Returns null if canvas is not available (Render free tier).
 */
 ...
 * @returns {Promise<AttachmentBuilder|null>} Discord attachment, or null if canvas unavailable.
 */
async function generateWelcomeImage(member, { message } = {}) {
  if (!CANVAS_AVAILABLE) {
    logger.info('canvas not available — skipping welcome image');
    return null;
  }
  ...
  try {
    ...
    try {
      ...
      // Avatar fetch failed — skip avatar
    }
    ...
    // ── Text content ────────────────────────────────────────────────
    const TEXT_X = avatarImage ? 260 : 60;
    ...
    ctx.beginPath();
    ctx.roundRect(badgeX, badgeY, badgeWidth, 28, 8);
    ctx.fill();
